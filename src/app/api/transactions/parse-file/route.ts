  ...
  try {
    ...
    if (file) {
      ...
      if (fileName.endsWith(".pdf")) {
        ...
        // For Excel files, convert to text first using xlsx library
        // We do this server-side to handle all edge cases
        try {
          ...
          return NextResponse.json(
            { error: "Failed to read Excel file. Please ensure it is a valid .xlsx or .xls file." },
            { status: 400 }
          );
        }
